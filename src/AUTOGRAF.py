...
found_major_version = ".".join(app.version.split('.')[:2])
...
def initialBundleblock():
    ...
    for currentFolder, folderName in zip(subfolderList, folderNames):
        ...
        chunk.matchPhotos(downscale=1, generic_preselection=True, reference_preselection=True, keep_keypoints=True, tiepoint_limit=3000, keypoint_limit=25000) # perform initial bundleblock
...
def incrementalSfM(downScalingImageMatching):
    ...
    chunk.matchPhotos(downscale=downScalingImageMatching, generic_preselection=True, reference_preselection=True, keypoint_limit=25000, tiepoint_limit= 3000, keep_keypoints=True)
    ...
    for chunk in doc.chunks:
        try:
            if chunk.label not in folderGroupNames:
                ...
                chunk.buildDepthMaps(downscale=downScalingDenseMatching)
...
def ransacINDIGO(X,Y,Z, camCoords, chunk):
    ...
    model_robust, inliers = ransac(data, LineModelND, min_samples=2, residual_threshold=0.1, max_trials=1000)
    outliers = inliers == False
...
def about():
    app.messageBox("AUTOGRAF - AUTomated Orthorectification of GRAFfiti photos. \n"
...
def runAll():
    ...
    '''
    global downScalingDenseMatching
    downScalingDenseMatching = 4
    matchingDownscaling = 1
    ...
    print(datetime.datetime.now())
    incrementalSfM(matchingDownscaling)
